...
/*   Updated: 2022/12/20 16:31:05 by jvan-tol      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_infile_out(char *input, t_lexer *lexer)
{
	...
	if (lexer->type == INFILE && input[lexer->index] == '<' \
		...
		lexer->next->type = INFILE;
			...
			ft_strncmp(str, "<<", 3) == 0 && lexer->next->type \
			!= OUTFILE_APPEND && lexer->next->type != OUTFILE)
		lexer->next->type = HERE_DOC;
	else if (lexer->type == OUTFILE && input[lexer->index] == '>' \
			&& lexer->next->type != INFILE && lexer->next->type != HERE_DOC)
		lexer->next->type = OUTFILE;
	else if (lexer->type == OUTFILE_APPEND && \
			ft_strncmp(str, ">>", 3) == 0 && lexer->next->type != HERE_DOC \
			&& lexer->next->type != INFILE)
		lexer->next->type = OUTFILE_APPEND;
	free(str);
	return (1);
...
void	post_process(char *input, t_lexer *lexer)
{
	...
	{	
		if (lexer->next == NULL)
			return ;
		if (!check_infile_out(input, lexer))
			return ;
		if (!check_cmd_args(lexer))
			return ;
